...
// Add global keyframes for float-spin animation
if (typeof document !== 'undefined' && !document.getElementById('float-spin-keyframes')) {
  ...
  useEffect(() => {
    async function fetchCategories() {
      ...
      try {
        ...
        // Only show up to 6 categories whose item count is more than 4
        apiCategories = apiCategories.filter(cat => Array.isArray(cat.items) && cat.items.length > 4).slice(0, 6);
        // Map to UI structure
        setCategories(apiCategories.map(cat => ({
          id: cat._id,
          name: cat.name,
          image: cat.image,
          subcategories: Array.isArray(cat.items) ? cat.items.slice(0, 4).map(item => ({
            id: item.id,
            name: item.ItemName,
            image: item.image // fallback to category image, or use a default if needed
          })) : []
        })));
      } catch (err) {
        ...
        >
          ...
          <div className="relative h-full transform transition-all duration-300 hover:scale-105 hover:shadow-2xl"
            ...
            <div className="absolute right-0 top-0 bottom-0 w-2/3 flex items-center justify-between p-6">
              <h2 className={`text-white text-xl font-bold leading-tight transition-all duration-300 ${
                isHovered ? 'scale-105' : ''
              }`}>
                {category.name}
              </h2>
              ...
              {category.subcategories.map((subcategory) => (
                <div 
                  ...
                  style={{ 
                    ...
                    borderColor: '#8e191c',
                    borderWidth: '1px',
                    // Optionally, you can add a hover effect for border color
                  }}
