...
const AddBanner = ({ onBack, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    const fetchStores = async () => {
      try {
        ...
        // Try to extract array from possible response shapes
        let storesArr = [];
  ...
  const storeOptions = Array.isArray(stores)
    ? stores.map(store => ({
        ...
        label: store.name || store.storeName || 'Unnamed Store',
      }))
    : [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await superAdminBannerService.createBanner({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-red-50/30 to-pink-50/20 p-4">
      {loading && <LoaderOverlay text="Uploading Banner..." />}
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1 text-base">Create stunning banners for your website</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200/50 p-6 shadow-xl">
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold text-gray-700 mb-2" htmlFor="store-select">
                  Select Store <span className="text-red-500">*</span>
                </label>
                <Select
                  id="store-select"
                  options={storeOptions}
                  value={storeOptions.find(opt => opt.value === selectedStoreId) || null}
                  onChange={handleStoreChange}
                  isClearable
                  isSearchable
                  isLoading={storesLoading}
                  placeholder="-- Select a Store --"
                  noOptionsMessage={() => storesLoading ? 'Loading stores...' : 'No stores found'}
                  classNamePrefix="react-select"
                  styles={{
                    control: (base, state) => ({
                      ...base,
                      borderColor: !selectedStoreId && state.isFocused ? '#ef4444' : base.borderColor,
              ...
              <div className="flex items-center space-x-3 mb-6">
                ...
                <h3 className="text-xl font-semibold text-gray-900">Banner Upload</h3>
              </div>
              ...
              {imagePreview ? (
                ...
                >
                  ...
                  <h4 className="text-xl font-bold text-gray-900 mb-2 transition-all duration-300 group-hover:text-red-700">
                    {dragActive ? 'Drop your banner here' : 'Upload Banner Image'}
                  </h4>
              ...
              {/* Upload Guidelines - Always Visible */}
              <div className="mt-6 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200 p-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-blue-800">High resolution (1920x1080px+)</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Professional imagery</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Proper lighting & contrast</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Brand-relevant content</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Clear call-to-action text</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Mobile-friendly design</span>
                  </div>
                </div>
              </div>
            </div>
