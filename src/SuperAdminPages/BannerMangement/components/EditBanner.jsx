...
import bannerService from '../../../services/SuperAdmin/BannerService';
import LoaderOverlay from '../../../components/LoaderOverlay';

const EditBanner = ({ banner, onBack, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  const [imagePreview, setImagePreview] = useState(banner?.image || null);
  const [selectedImage, setSelectedImage] = useState(null);
  const [dragActive, setDragActive] = useState(false);
  const [bannerType, setBannerType] = useState(banner?.bannerType || 'promotional');
  const [isVisible, setIsVisible] = useState(banner?.isVisible !== false);

  const bannerTypes = [
    { value: 'promotional', label: 'Promotional', icon: FiTrendingUp, color: 'from-red-500 to-red-600', description: 'Marketing campaigns' },
    { value: 'announcement', label: 'Announcement', icon: FiMessageSquare, color: 'from-red-600 to-red-700', description: 'Important updates' },
    { value: 'featured', label: 'Featured', icon: FiStar, color: 'from-red-700 to-red-800', description: 'Special content' },
    { value: 'seasonal', label: 'Seasonal', icon: FiGift, color: 'from-red-500 to-pink-600', description: 'Holiday content' },
    { value: 'advertisement', label: 'Advertisement', icon: FiDollarSign, color: 'from-red-800 to-red-900', description: 'Sponsored ads' }
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    // Allow submission if either image is changed OR settings are changed
    if (!selectedImage && bannerType === (banner?.bannerType || 'promotional') && isVisible === (banner?.isVisible !== false)) {
      toast.error('Please make some changes to update the banner');
      return;
    }
    ...
    try {
      ...
      console.log('Calling bannerService.updateBanner with id:', banner._id);

      const response = await bannerService.updateBanner(banner._id, formData);
      ...
      const errorMessage = error.response?.data?.message || 'Failed to update banner. Please try again.';
  ...
  if (!banner) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-red-50/30 to-pink-50/20 p-6 flex items-center justify-center">
        <div className="text-center py-16 bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-200/50 p-12 shadow-xl">
          <FiImage className="w-16 h-16 text-gray-600 mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-gray-400 mb-2">Banner not found</h3>
          <p className="text-gray-500">The banner you're looking for doesn't exist</p>
          <button
            onClick={onBack}
            className="mt-4 px-6 py-2 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-300"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-red-50/30 to-pink-50/20 p-4">
      {loading && <LoaderOverlay text="Updating Banner..." />}
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1 text-base">Update your banner image and settings</p>
            </div>
          </div>
          
          <div className="flex items-center space-x-3">
            <div className="px-4 py-2 rounded-full bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-500/20 text-red-600 text-sm font-medium">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-200/50 p-6 shadow-xl">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <h3 className="text-xl font-semibold text-gray-900">Update Banner Image</h3>
              </div>
              ...
              {selectedImage ? (
                  ...
                  <div className="mt-4 p-4 rounded-xl bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-medium text-green-800">New image ready</span>
                      </div>
                ...
                >
                  ...
                  <h4 className="text-xl font-bold text-gray-900 mb-2 transition-all duration-300 group-hover:text-red-700">
                    {dragActive ? 'Drop new banner here' : 'Upload New Banner'}
                  </h4>
                  <p className="text-gray-600 text-center mb-4 max-w-md text-sm">
                    Drag and drop your new banner image here, or click to browse
                  </p>
                  <div className="flex items-center space-x-4 text-xs text-gray-500">
                    ...
                    <span className="px-2 py-1 bg-white rounded-full border text-xs">1920x1080px</span>
                  </div>
              ...
              <div className="mt-6 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200 p-4">
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <h4 className="text-sm font-semibold text-blue-900">Update Guidelines</h4>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Choose high-quality replacement</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Maintain consistent branding</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Consider current placement</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <FiCheck className="w-3 h-3 text-green-600 flex-shrink-0" />
                    <span className="text-blue-800">Ensure design consistency</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-200/50 p-6 shadow-xl">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <h3 className="text-lg font-semibold text-gray-900">Banner Type</h3>
              </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-200/50 p-6 shadow-xl">
              <div className="flex items-center space-x-3 mb-6">
                <div className="p-2 rounded-xl bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-500/20">
                  {isVisible ? <FiEye className="w-5 h-5 text-red-600" /> : <FiEyeOff className="w-5 h-5 text-red-600" />}
                </div>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <div className={`text-sm mt-1 font-medium ${
                        isVisible ? 'text-white/90' : 'text-gray-500'
                      }`}>
                        {isVisible ? 'Banner will be shown publicly' : 'Banner will be hidden from users'}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {/* Comparison Section */}
        {selectedImage && (
          <div className="mt-8 bg-white/80 backdrop-blur-sm rounded-3xl border border-gray-200/50 p-8 shadow-xl">
            <h3 className="text-xl font-semibold text-gray-900 mb-6 flex items-center space-x-3">
              <div className="p-2 rounded-xl bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border border-purple-500/20">
        ...
        <div className="flex justify-end gap-4 pt-6">
          ...
          <button
            ...
            disabled={loading || (!selectedImage && bannerType === (banner?.bannerType || 'promotional') && isVisible === (banner?.isVisible !== false))}
